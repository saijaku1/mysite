  ...
  function spawnEnemy() {
    ...
    const enemy = {
      ...
      lastShotTime: 0,
      shotInterval: enemyBulletIntervalMin + Math.random()*(enemyBulletIntervalMax - enemyBulletIntervalMin)
    };
    enemies.push(enemy);
  }
  ...
  function isColliding(a, b) {
    const ax = a.x, ay = a.y, aw = a.width || 4, ah = a.height || 10;
  ...
  function gameOver() {
    gameRunning = false;
    messageElem.textContent = 'ゲームオーバー！スコアをランキングに登録しました。';
    if(enemySpawnTimer) clearInterval(enemySpawnTimer);
  ...
  function showRanking() {
    ...
    ranking.forEach((item, i) => {
      ...
      li.textContent = `${i+1}位: ${item.score}点 (${date.toLocaleDateString()} ${date.toLocaleTimeString()})`;
      ul.appendChild(li);
    });
